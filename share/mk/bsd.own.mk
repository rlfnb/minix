...
.if defined(__MINIX)
# Force static linking, disable PIC and shared libraries
LDSTATIC?=	-static
MKDYNAMICROOT?=	no
MKPIC:=		no
MKPICLIB:=	no
...
# Disable features MINIX does not support
MKPROFILE:=	no
MKKMOD:=	no
MKRUMP:=	no
MKCOMPAT:=	no
...
.if defined(CONFIG_SMP) && ${CONFIG_SMP} == "yes"
...
# ARM unaligned access
.if !empty(MACHINE_ARCH:Mearm*)
CFLAGS+= -mno-unaligned-access
.endif

# Native MINIX host detection
.  if !defined(USETOOLS) || empty(USETOOLS:M[Yy][Ee][Ss])
__uname_s!= uname -s
.    if ${__uname_s} == "Minix"
USETOOLS?=	no
MACHINE?=	${__uname_s:tl}
MACHINE_ARCH?=	i386
.      if exists(/usr/pkg/bin/clang)
HOST_CC?=	/usr/pkg/bin/clang
HOST_CXX?=	/usr/pkg/bin/clang++
.      endif
HOST_CPPFLAGS?= -D_NETBSD_SOURCE
HOST_SH?=	/bin/sh
MAKEDIRTARGET?=	${MAKE} -C
.    endif # ${__uname_s} == "Minix"
.  endif # USETOOLS

# MINIX MK variables set directly (not through _MKVARS loops
# which use syntax too complex for the bootstrapped nbmake)
MKSYSDEBUG?=	yes
MKLIVEUPDATE?=	yes
MKLLVMCMDS?=	yes
...
# MINIX: Override target triple for MINIX
.if defined(__MINIX)
...
#MINIX-specific USE_* vars
.if defined(__MINIX)
.for _v in WATCHDOG ACPI PAE APIC DEBUGREG SYSDEBUG LIVEUPDATE PCI
.if ${MK${_v}:Uno} == "no"
USE_${_v}:= no
.else
USE_${_v}?= yes
